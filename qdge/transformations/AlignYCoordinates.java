 ...
 * @author nvcleemp
 */
public class AlignYCoordinates extends AbstractUniversalSelectionMovementGraphTransformation implements HasReferenceVertex {
    
    private Vertex reference;

    public AlignYCoordinates(GraphSelectionModel selectionModel) {
    ...
    public GraphTransformation repeatTransformation(Graph g) {
        ...
        getVertices(g).stream().forEach(v -> {
            repeat.setPosition(v, v.getX(), reference.getY());
        });
        return repeat;
    }
